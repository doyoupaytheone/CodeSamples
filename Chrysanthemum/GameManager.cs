...
public class GameManager : MonoSingleton<GameManager>
{
    ...
    {
        //Collects save name data
        string[] names = new string[_saveSlots.Length];
        for (int i = 0; i < _saveSlots.Length; i++)
            names[i] = _saveSlots[i].SaveName;

        //Collects save levels data
        int[] levels = new int[_saveSlots.Length];
        for (int i = 0; i < _saveSlots.Length; i++)
            levels[i] = _saveSlots[i].FurthestLevelIndex;
        ...
        if (currentGameState == GameState.MainMenu)
        {
            ...
            _menuManager.LoadMenuSettings(names, times, levels);
        }
    }
    ...
    //Changes the scene to the furthest level reached in a save slot
    public void ChangeSceneUsingSaveSlot(int saveIndex)
    {
        int sceneIndex = _saveSlots[saveIndex].FurthestLevelIndex;
        if (sceneIndex == 0)
    ...
    {
        //Checks to see if this is the furthest level on the save slot
        //If so, it saves the new furthest level data
        if (sceneIndex > _saveSlots[_currentSaveSlot].FurthestLevelIndex)
            _saveSlots[_currentSaveSlot].FurthestLevelIndex = sceneIndex;
    ...
    public void ToNextScene()
    {
        ...
        {
            //Checks to see if the next scene is the furthest level on the save slot
            //If so, it saves the new furthest level data
            if (current + 1 > _saveSlots[_currentSaveSlot].FurthestLevelIndex)
                _saveSlots[_currentSaveSlot].FurthestLevelIndex = current + 1;
    ...
    public bool SelectSaveSlot(int index)
    {
        ...
        if (_saveSlots[index].PlayTime == 0 && _saveSlots[index].SaveName == "")
            return true;
        else
            return false;
    }

    //Renames the currently selected save file
    public void RenameSaveSlot(string saveName)
    {
        _saveSlots[_currentSaveSlot].SaveName = saveName;
        SaveGame();
    }
    ...
    public void DeleteSaveData(int slotIndex)
    {
        ...
        _saveSlots[slotIndex].FurthestLevelIndex = 0;
        _saveSlots[slotIndex].SaveName = "";
        _saveSlots[slotIndex].PlayTime = 0;
        _saveSlots[slotIndex].TotalSpellsAquired = 0;
